...
void drawRectDMA(int x, int y, int width, int height, volatile u16 color) { // draw a rectangle for specified parameters
...
void drawFullScreenImageDMA(const u16 *image) { // draw an image over the entire screen
    DMA[3].src = image;
    DMA[3].dst = videoBuffer;
    DMA[3].cnt = (WIDTH * HEIGHT) | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
}

void drawImageDMA(int x, int y, int width, int height, const u16 *image) { // draw an image to specific proportions
    for (int i = 0; i < height; i++) {
...
void fillScreenDMA(volatile u16 color) { // fill screen with one color
    DMA[3].src = &color;
    DMA[3].dst = videoBuffer;
